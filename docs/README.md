...
- [x] `예외처리` 페어 매칭 정보가 이미 존재하면, 다시 진행을 확인한다
    - [x] `아니오` 입력 시 [과정, 레벨, 미션 선택 기능] 다시 진행
- [x] `예외처리` 이미 페어를 진행한 크루끼리 매칭 된다면 다시 매칭을 진행한다
    - [x]] `예외처리` 3회 시도 후 매칭이 안되면 예외가 발생한다
